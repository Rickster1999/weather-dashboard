...
$("#currentCity").text(
  "Today's Date" + " - " + currentDate.format("MMM D, YYYY")
);
...
$("#cityInputBtn").on("click", function () {
  ...
  inputCity =
    inputCityFormat.charAt(0).toUpperCase() + inputCityFormat.slice(1);
...
function getCoords(inp) {
  var reqLatLon =
    ...
    inp +
    "&appid=0dfeca27786a8a8c837f120f88c9a791";
  fetch(reqLatLon)
...
function getCurrent(lat, lon) {
  var reqCurWea =
    ...
    "&lon=" +
    lon +
    "&units=imperial&appid=0dfeca27786a8a8c837f120f88c9a791";
  fetch(reqCurWea)
    .then(function (response) {
...
function popCurrent(data1) {
  ...
  $("#currentWind").text(
    "Wind: " +
      Math.round(data1.wind.speed) +
      " - " +
      getDirection(data1.wind.deg)
  );
  $("#currentHumid").text("Humidity: " + data1.main.humidity + "%");
  $("#currentIcon").attr({
    src: "https://openweathermap.org/img/wn/" + data1.weather[0].icon + ".png",
...
function popFiveDay(data2) {
  ...
  for (let i = 0; i < data2.list.length; i++) {
    if (data2.list[i].dt_txt.includes("18:00:00") === true) {
      $("#fiveTemp" + iter).text(
        "Temp: " + Math.round(data2.list[i].main.temp) + "° F"
      );
      $("#fiveWind" + iter).text(
        "Wind: " +
          Math.round(data2.list[i].wind.speed) +
          " - " +
          getDirection(data2.list[i].wind.deg)
      );
      $("#fiveHumid" + iter).text(
      ...
      $("#fiveIcon" + iter).attr({
        src:
          ...
          data2.list[i].weather[0].icon +
          ".png",
        alt: data2.list[i].weather[0].description,
      });
      iter++;
    }
  }
...
function addPrevSearch(inp) {
  var prevSearchEl = $(
    '<button class="button is-fullwidth is-link is-small mb-1 prevButton">' +
      inp +
      "</button>"
  );
  $("#previousSearches").append(prevSearchEl);
